...
<body>
    ...
    //si la sesión no está iniciada, la inicio.
        ...
        //si la no existe el array nombre1 inicio el contador
                    ...
                    <select id="select" multiple="multiple" onchange="submit()" name="alumnos">
                        ...
                        echo $contadorAlumnos;
                            for($i=1; $i<$contadorAlumnos; $i++){
                                echo "<option value=".$i.">".$_SESSION['nombre'.$i]."</option>";
                            }
                        ?>
                    </select>
            ...
            <div class="colAbajo">
                <textarea readonly="readonly"><?php 
                    if(isset($_SESSION['seleccionado'])){
                        echo $_SESSION['nombre'.$_SESSION['seleccionado'].'']."\n";                
                        echo $_SESSION['apellido'.$_SESSION['seleccionado'].'']."\n";
                        echo $_SESSION['tlfn'.$_SESSION['seleccionado'].'']."\n";
                        echo $_SESSION['email'.$_SESSION['seleccionado'].'']."\n";
                        echo $_SESSION['curso'.$_SESSION['seleccionado'].''];
                    }
                ?>
                </textarea>
            </div>
